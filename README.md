# Backend de autenticación JWT con API REST para Node.js [![Estado de la compilación]
(https://jenkins.wmtech.cc/job/nodejs-jwt-auth/job/main/badge/icon)]
(https://jenkins.wmtech.cc/job/nodejs-jwt-auth/job/main/)
Backend de autenticación JWT con API REST desarrollado con Node.js y Express. Este código utiliza MongoDB como base de datos. Se admite Docker.

Utilizamos dos colecciones: una para usuarios y otra para tokens. Se admite la actualización de tokens; los tokens caducados se descartan mediante el índice TTL de MongoDB.

La caducidad de los tokens de acceso y de actualización se puede configurar en los archivos `controllers/auth.js` y `models/token.js`.

Tenga en cuenta que deberá recrear el índice de la colección si la actualiza.

También se implementa la rotación de claves; las claves de firma se cambian diariamente mediante una tarea programada (cron) que se puede modificar en [index.js].

Puede encontrar un ejemplo de middleware en [middlewares/is-auth.js], donde las aplicaciones de terceros solicitan el JWKS público para verificar los tokens.

Los nuevos usuarios deben verificar su correo electrónico mediante un enlace de verificación.

También se implementa el restablecimiento de contraseña por correo electrónico.
### Features
